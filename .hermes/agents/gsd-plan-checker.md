...
**Project skills:** Check `.hermes/skills/` or `.agents/skills/` directory if either exists:
...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/few-shot-examples/plan-checker.md
...
## Dimension 10: HERMES.md Compliance

**Question:** Do plans respect project-specific conventions, constraints, and requirements from HERMES.md?

**Process:**
1. Read `./HERMES.md` in the working directory (already loaded in `<project_context>`)
...
4. Flag plans that introduce patterns HERMES.md explicitly forbids
5. Flag plans that skip steps HERMES.md explicitly requires (e.g., required linting, specific test frameworks, commit conventions)

**Red flags:**
- Plan uses a library/pattern HERMES.md explicitly forbids
- Plan skips a required step (e.g., HERMES.md says "always run X before Y" but plan omits X)
- Plan introduces code style that contradicts HERMES.md conventions
- Plan creates files in locations that violate HERMES.md's architectural constraints
- Plan ignores security requirements documented in HERMES.md

**Skip condition:** If no `./HERMES.md` exists in the working directory, output: "Dimension 10: SKIPPED (no HERMES.md found)" and move on.
...
issue:
  ...
  severity: blocker
  description: "Plan uses Jest for testing but HERMES.md requires Vitest"
  plan: "01"
  task: 1
  claude_md_rule: "Testing: Always use Vitest, never Jest"
  plan_action: "Install Jest and create test suite..."
  fix_hint: "Replace Jest with Vitest per project HERMES.md"
```
...
issue:
  dimension: claude_md_compliance
  severity: warning
  description: "Plan does not include lint step required by HERMES.md"
  plan: "02"
...
- [ ] HERMES.md compliance checked (plans respect project conventions)
