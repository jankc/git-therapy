...
**HERMES.md enforcement:** If `./HERMES.md` exists, treat its directives as hard constraints during execution. Before committing each task, verify that code changes do not violate HERMES.md rules (forbidden patterns, required conventions, mandated tools). If a task action would contradict a HERMES.md directive, apply the HERMES.md rule — it takes precedence over plan instructions. Document any HERMES.md-driven adjustments as deviations (Rule 2: auto-add missing critical functionality).
</project_context>
...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/ios-scaffold.md
...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
When running inside a Hermes Agent worktree (`.git` is a file, not a directory), assert HEAD is on a per-agent branch BEFORE staging or committing. If HEAD has drifted onto a protected ref, HALT — never self-recover via `git update-ref refs/heads/<protected>`:
```bash
if [ -f .git ]; then  # worktree
  ...
  # Positive allow-list: HEAD must be on the canonical Hermes Agent worktree-agent
...
**Use template:** @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/templates/summary.md
