...
// Hermes Agent Statusline - GSD Edition
...
/**
 * Extract the most recently invoked slash command from a Hermes Agent JSONL
 * transcript file. Returns the command name (no leading slash) or null.
 *
 * Hermes Agent embeds slash invocations in user messages as
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Hermes Agent transcripts are plain identifiers like "gsd-plan-phase"
...
function runStatusline() {
  ...
  try {
    ...
    // Hermes Agent reserves a buffer for autocompact. By default this is ~16.5%
    ...
    const claudeDir = process.env.CLAUDE_CONFIG_DIR || path.join(homeDir, '.hermes');
