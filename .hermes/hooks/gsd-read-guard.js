...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Hermes Agent natively enforces read-before-edit — skip the advisory (#1984, #2344, #2520).
    //
    ...
    //      Reliable across Hermes Agent versions because it's schema, not env.
    //   2. `CLAUDE_CODE_ENTRYPOINT` / `CLAUDE_CODE_SSE_PORT` — env vars that
    //      Hermes Agent does propagate to hook subprocesses (verified on
    //      Hermes Agent CLI 2.1.116).
    //   3. `CLAUDE_SESSION_ID` / `CLAUDECODE` — kept for back-compat and in
    //      case future Hermes Agent versions propagate them to hook
