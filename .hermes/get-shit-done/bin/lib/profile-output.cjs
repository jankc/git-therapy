 ...
 *   - generate-claude-profile: Developer Profile section in HERMES.md
 *   - generate-claude-md: full HERMES.md with managed sections
 */
...
// HERMES.md fallback / placeholder text — runtime-aware so emitted slash
...
function buildClaudeMdFallbacks(runtime) {
  return {
    ...
    skills: 'No project skills found. Add skills to any of: `.hermes/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
  };
}

// Directories where project skills may live (checked in order)
const SKILL_SEARCH_DIRS = ['.hermes/skills', '.agents/skills', '.cursor/skills', '.github/skills', '.codex/skills'];
 ...
 * (name + description) for each. Returns a table summary for HERMES.md so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.claude', 'HERMES.md');
  } else if (options.output) {
    targetPath = path.isAbsolute(options.output) ? options.output : path.join(cwd, options.output);
  } else {
    // Read claude_md_path from config, default to ./HERMES.md
    let configClaudeMdPath = './HERMES.md';
    try {
      const config = loadConfig(cwd);
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  try {
    ...
    // regardless of claude_md_path, so Codex projects never write to HERMES.md.
