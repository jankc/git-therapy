...
**GSD** (Get Shit Done) creates hierarchical project plans optimized for solo agentic development with Hermes Agent.
...
- Generates `./.hermes/skills/spike-findings-[project]/` with references and sources
- Writes summary to `.planning/spikes/WRAP-UP-SUMMARY.md`
- Adds auto-load routing line to project HERMES.md
...
- Generates `./.hermes/skills/sketch-findings-[project]/` with design decisions, CSS patterns, HTML structures
...
- Saves timestamped note to `.planning/notes/` (or `/Users/jank/dev/personal/git-therapy/.hermes/notes/` globally)
...
Every topic output starts with a `**Topic:** \`<alias>\` → \`<heading>\` *(scope: full | compact)*` preamble so resolved routing is visible. See `get-shit-done/workflows/help/modes/topic.md` for the full alias table. Unknown topics print the recognized list.
...
- **`/gsd-ultraplan-phase [phase]`** — [BETA] Offload plan phase to Hermes Agent's ultraplan cloud; review in browser and import back.
