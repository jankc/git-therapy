...
visible in your runtime (Hermes Agent's `/context` slash command output, or the
...
<stale_task_cleanup>
**Windows-specific:** Check for stale Hermes Agent task directories that accumulate on crash/freeze.
...
TASKS_DIR="/Users/jank/dev/personal/git-therapy/.hermes/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/jank/dev/personal/git-therapy/.hermes/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/jank/dev/personal/git-therapy/.hermes/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
