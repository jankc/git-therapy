...
- ./HERMES.md (if exists — project-specific guidelines)
...
- ./HERMES.md (if exists — follow project-specific guidelines)
...
Step 1 — HEAD attachment assertion (MANDATORY, runs before any reset/commit):
  ...
  if ! echo "$ACTUAL_BRANCH" | grep -Eq '^worktree-agent-[A-Za-z0-9._/-]+$'; then
    echo "FATAL: worktree HEAD '$ACTUAL_BRANCH' is not in the worktree-agent-* namespace (Hermes Agent's per-agent worktree branch namespace)." >&2
...
- .planning/STATE.md (Project state)
- ./HERMES.md (Project instructions, if exists)
- .hermes/skills/ or .agents/skills/ (Project skills, if either exists — list skills, read SKILL.md for each, follow relevant rules during implementation)
</files_to_read>
...
**Known Hermes Agent bug (classifyHandoffIfNeeded):** If executor reports "failed" with error `classifyHandoffIfNeeded is not defined`, this is a Hermes Agent runtime bug — not a real failure. Check if summary file exists and git log shows commits. If so, treat as successful.
