...
- **Hermes Agent:** Uses `Agent(subagent_type="gsd-executor", ...)` — blocks until complete, returns result
...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/agent-contracts.md
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/context-budget.md
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/gates.md
</required_reading>

<available_agent_types>
These are the valid GSD subagent types registered in .hermes/agents/ (or equivalent for your runtime).
...
```bash
_GSD_SHIM_NAME="gsd-tools.cjs"; _GSD_RUNTIME_ROOT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"; GSD_TOOLS="${_GSD_RUNTIME_ROOT}/get-shit-done/bin/${_GSD_SHIM_NAME}"; if [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; elif [ -f "${_GSD_RUNTIME_ROOT}/.hermes/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="${_GSD_RUNTIME_ROOT}/.hermes/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; elif command -v gsd-tools >/dev/null 2>&1; then GSD_TOOLS="$(command -v gsd-tools)"; gsd_run() { "$GSD_TOOLS" "$@"; }; elif [ -f "/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; else echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS and gsd-tools is not on PATH. Run: npx -y @opengsd/gsd-core@latest --claude --local" >&2; exit 1; fi
...
**Model resolution:** If `executor_model` is `"inherit"`, omit the `model=` parameter from all `Agent()` calls — do NOT pass `model="inherit"` to Agent. Omitting the `model=` parameter causes Hermes Agent to inherit the current orchestrator model automatically. Only set `model=` when `executor_model` is an explicit model name (e.g., `"claude-sonnet-4-6"`, `"claude-opus-4-7"`).
...
```
Codex maps subagents to `spawn_agent`, which has no direct Codex mapping for Hermes Agent's `isolation="worktree"` parameter. Failing closed prevents main-checkout edits while the workflow believes agents are isolated.
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/workflows/execute-plan.md` **inline**
...
between a large tool_result and the next assistant turn (seen on Hermes Agent
...
3. **Spawn executor agents:**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     # Hermes Agent inherits the orchestrator model automatically.
     ...
     prompt="
       ...
       spawned by Hermes Agent's `isolation="worktree"` use the `worktree-agent-<id>`
       ...
       prompt; silent bypass violates project HERMES.md guidance (#2924).
       ...
       <execution_context>
       @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/workflows/execute-plan.md
       @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/templates/summary.md
       @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/checkpoints.md
       @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/tdd.md
       @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/executor-examples.md'}
       </execution_context>
       ...
       ` : ''}
       - ${PROJECT_ROOT}/HERMES.md (Project instructions, if exists — follow project-specific guidelines and coding conventions)
       - ${PROJECT_ROOT}/.hermes/skills/ or ${PROJECT_ROOT}/.agents/skills/ (Project skills, if either exists — list skills, read SKILL.md for each, follow relevant rules during implementation)
       </files_to_read>

       ${AGENT_SKILLS}

       <mcp_tools>
       If HERMES.md or project instructions reference MCP tools (e.g. jCodeMunch, context7,
...
4. **Wait for all agents in wave to complete.**
   ...
   **This fallback applies automatically to all runtimes.** Hermes Agent's Agent() normally
...
Read and follow `/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
...
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Hermes Agent bug, not GSD. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
