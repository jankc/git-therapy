...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/ui-brand.md
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/revision-loop.md
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/gate-prompts.md
...
Set `TEXT_MODE=true` if `--text` is present in $ARGUMENTS OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `AskUserQuestion` call with a plain-text numbered list and ask the user to type their choice number. This is required for Hermes Agent remote sessions (`/rc` mode) where TUI menus don't work through the Claude App.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/skeleton-template.md`.
...
**Project instructions:** Read ./HERMES.md if exists — follow project-specific guidelines
**Project skills:** Check .hermes/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, research should account for project skill patterns
</additional_context>
...
**If found:**
1. Read template: `/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.hermes/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS_PATH=$(ls ./.hermes/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
```
...
**Project instructions:** Read ./HERMES.md if exists — follow project-specific guidelines
**Project skills:** Check .hermes/skills/ or .agents/skills/ directory (if either exists) — read SKILL.md files, plans should account for project skill rules

${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
**Project instructions:** Read ./HERMES.md if exists — verify plans honor project guidelines
**Project skills:** Check .hermes/skills/ or .agents/skills/ directory (if either exists) — verify plans account for project skill rules
</verification_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
...
stdio deadlocks with MCP servers — see Hermes Agent issue anthropics/claude-code#28126):
...
3. **Clean up stale task directories:**
   ```powershell
   # Remove stale subagent task dirs (Hermes Agent never cleans these on crash)
   Remove-Item -Recurse -Force "$env:USERPROFILE\.claude\tasks\*" -ErrorAction SilentlyContinue
   ```
4. **Reduce MCP server count:** Temporarily disable non-essential MCP servers in settings.json
5. **Retry:** Restart Hermes Agent and run `/gsd-plan-phase` again
