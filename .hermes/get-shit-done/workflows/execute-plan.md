...
@/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/git-integration.md
</required_reading>
...
3. **Critical ordering — write and commit SUMMARY.md as one atomic block.** Do NOT
   ...
   **Known Hermes Agent bug (classifyHandoffIfNeeded):** If any segment agent reports "failed" with `classifyHandoffIfNeeded is not defined`, this is a Hermes Agent runtime bug — not a real failure. Run spot-checks; if they pass, treat as successful.
...
2. **MCP tools:** If HERMES.md or project instructions reference MCP tools (e.g. jCodeMunch for code navigation), prefer them over Grep/Glob when available. Fall back to Grep/Glob if MCP tools are not accessible.
3. Per task:
...
See `/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
bypass violates project HERMES.md guidance. If a project explicitly opts out via
...
See /Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.hermes/get-shit-done/workflows/node-repair.md` with:
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Users/jank/dev/personal/git-therapy/.hermes/get-shit-done/templates/summary.md`.
