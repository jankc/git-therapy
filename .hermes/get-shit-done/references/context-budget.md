...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever GSD itself can save through `model_profile` tuning. This is a Hermes Agent harness concern, not a GSD concern: GSD does **not** manage MCP enablement. The toggle lives in `.hermes/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
The keys live in `.hermes/settings.json` (project) or `/Users/jank/dev/personal/git-therapy/.hermes/settings.json` (global) — **not** in `.planning/config.json`:
